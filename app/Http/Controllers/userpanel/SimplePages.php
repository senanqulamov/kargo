...
class SimplePages extends Controller
{
    ...
    public function support()
    {
        ...
        $support_demands = Support_demand::where('user_id', Auth::user()->id)->orderBy('created_at', 'DESC')->get();
        $support_messages = Support_message::where('user_id', Auth::user()->id)->orderBy('support_id', 'DESC')->get();
    ...
    public function support_demand(Request $request)
    {
        ...
        return Redirect::back()->with('message', 'Your support demand succesfully sent');
    }
    ...
    public function calculator()
    {
        ...
        return view('userpanel.helpers.calculator', compact('cargo_companies', 'countries'));
    }
    ...
    public function notifications()
    {
        ...
        return view('userpanel.helpers.notifications', compact('notifications'));
    }
    ...
    public function buyforme()
    {
        ...
        $forme_orders = Forme_request::where('user_id', Auth::user()->id)->get();
    ...
    public function post_buyforme(Request $request)
    {
        ...
        if ($validator->fails()) {
            return Redirect::back()->with('error', 'Fill all needed inputs to be able send order request');
        }
        ...
        return Redirect::back()->with('message', 'Buy For Me order successfully sent');
    }
    ...
    public function create_bulk_order(Request $request)
    {
        ...
        for ($i = 1; $i < count($excel->rows()); $i++) {
            ...
            $packages[] = $package_id;
            $total_volume = ($excel->rows()[$i][18] * $excel->rows()[$i][19] * $excel->rows()[$i][20]) / 5000;


            $data = new stdClass();
            $data->country = $excel->rows()[$i][1];
            $data->total_deci = max($total_volume / 5000, $excel->rows()[$i][20]);
            ...
            $data->total_worth = $excel->rows()[$i][16] * $excel->rows()[$i][23] * $excel->rows()[$i][25];

            $additional_services = json_encode($this->service_calculation($data), true);
            $company_values = json_encode($this->company_calculation($data), true);
            ...
            $order_request = array(
                ...
                'total_deci' => max($total_volume / 5000, $excel->rows()[$i][20]),
                'total_count' => $excel->rows()[$i][16] * $excel->rows()[$i][23],
                'total_worth' => $excel->rows()[$i][16] * $excel->rows()[$i][23] * $excel->rows()[$i][25],
        ...
        return Redirect::back()->with('message', 'Bulk order successfully sent');
    }
